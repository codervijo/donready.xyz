...
`proj=donready.xyz`, so these all work either from this dir or from `sites/`:

| Command | What it does |
|---|---|
| `make buildsh` *(from `sites/`)* | Drop into a bash shell inside the docker container at `/usr/src/app` (= `sites/` mounted in). |
| `make run` *(from here)* / `make run proj=donready.xyz` *(from `sites/`)* | `pnpm install` then start dev server (auto-detected). |
| `make check-vite proj=donready.xyz` | Start the dev server, skipping install. |
| `make test proj=donready.xyz` | `pnpm install` + `pnpm build` + `pnpm test`. **Hard-fails outside docker** — `make buildsh` first, or `docker exec`. |
...
docker exec -w /usr/src/app <name> make test proj=donready.xyz
```
...
- **Deploy:** `wrangler deploy` (locally) or via Cloudflare's Git integration on push.
  ...
  `cd ../portfolio && make run ARGS="deploy donready.xyz"` runs `gh repo create` and
...
- **Live URL:** https://donready.xyz/  *(update once first deploy succeeds)*
...
`portfolio project status donready.xyz` (run from `sites/portfolio/`).
